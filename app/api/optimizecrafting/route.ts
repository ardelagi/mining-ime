  ...
  while (true) {
    ...
    for (const step of sortedSteps) {
      ...
      for (const req of step.requirements) {
        ...
        if (inv[req.item] < 0) inv[req.item] = 0;
      }
    ...
    if (!hasValuableMaterials && bestProfitMargin < 0.01) break;
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const updatedMiningData = applyCustomPrices(typedMiningData, customPrices);
